...
@click.option("--timeout", "-t", default=1.0, type=float, help="Connection timeout in seconds")
...
def ipcheck(target, ports, timeout, banner, blacklist):
    """
    Perform advanced check for an IP or hostname:
    - Validate IP
    - Resolve hostname -> IP
    - Reverse DNS lookup
    - Port scan (optional banner grabbing)
    - DNSBL blacklist check (optional)
    """
    ...
    open_ports = scanner.scan_ports(ip, ports=port_list, timeout=timeout)
    if open_ports:
        ...
        for p in open_ports:
            b = ""
            if banner:
                b = scanner.grab_banner(ip, p, timeout=timeout)
